...
function handleInputResume() {
  ...
  const addResumeCard = insertResumeCard(
    id,
    availability,
    age,
    location,
    ...
    if (resumeString) {
      ...
      if (Array.isArray(initialResume)) {
        ...
        initialResume.forEach((resume) => {
          identityHTML += insertIdentityCard(
            resume.id,
            resume.identity.name,
            resume.identity.role
          );
          resumeHTML += insertResumeCard(
            resume.id,
...
function setResumeToForm() {
  ...
  if (resumeString) {
    ...
    if (selectedResume) {
      ...
      document.getElementById("add-availability").value =
        selectedResume.resume.availability;
      document.getElementById("add-age").value = selectedResume.resume.age;
      document.getElementById("add-location").value =
        selectedResume.resume.location;
      document.getElementById("add-experience").value =
        selectedResume.resume.experience;
...
function displayResumeData() {
  ...
  if (resumeIndex !== -1) {
    ...
    document.getElementById("resume-name").textContent =
      resumeData[resumeIndex].identity.name;
    document.getElementById("resume-role").textContent =
      resumeData[resumeIndex].identity.role;
    document.getElementById("resume-availability").textContent =
      resumeData[resumeIndex].resume.availability;
    document.getElementById("resume-age").textContent =
      resumeData[resumeIndex].resume.age;
    document.getElementById("resume-location").textContent =
      resumeData[resumeIndex].resume.location;
    document.getElementById("resume-experience").textContent =
      resumeData[resumeIndex].resume.experience;
    document.getElementById("resume-email").textContent =
      resumeData[resumeIndex].resume.email;
  } else {
...
addResumeBtn.addEventListener("click", function () {
  ...
  if (resumeIndex !== -1) {
    ...
    id = resumeData.length > 0 ? resumeData[resumeData.length - 1].id + 1 : 1; // Increment ID
    handleInputResume();
  }
});
